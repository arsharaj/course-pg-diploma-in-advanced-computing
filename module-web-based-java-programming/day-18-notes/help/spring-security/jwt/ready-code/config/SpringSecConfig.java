...
//		cors().
//		and().
		csrf().disable().authorizeHttpRequests() // simply tell spring sec , to authorize all reqs
		.antMatchers("/products/view", "/swagger*/**", "/v*/api-docs/**", "/users/sign**").permitAll() // no authentication required
		...
		.anyRequest().authenticated() // any other remaining end points must be authenticated
		.and() // bridge
	...
	// configure Auth manager bean , required as a dependency in AuthController
	@Bean
