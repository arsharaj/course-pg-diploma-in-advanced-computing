...
1.  Type class in HashSet must override : hashCode & equals method both in consistent manner (contract must be fulfilled between equals and hashCode method).
...
String class , Wrapper classes , Date related classes have already followed this contract.
That is if 2 refs are equal via equal method then they must return the same hashcode values.
Converse need is not mandatory but recommended for better performance.
...
When the linked list size exceed the threshold defined in the java then this linked list is converted into a red-black tree to ensure constant time complexity.
