    ...
    return (
      ...
      <div className="row">
        ...
        <h2>flag : {this.state.flag}</h2>
                ...
                {this.state.searcharr.map(prod=><ProductDetails key={prod.pid} product={prod} modifyproduct={this.editproductdata}></ProductDetails>)}
        </div>
      </div>
      <div className='row'>
        {this.state.flag?<ProductFrom chflag={this.resetflag} editprod={this.state.upprod}></ProductFrom>:""}
